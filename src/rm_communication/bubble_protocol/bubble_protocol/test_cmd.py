...
class CmdVelPublisher(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f"Publishing to /cmd_vel: vx=0.1, vy=0.1, vw=-0.1 (10Hz)")
    ...
    def timer_callback(self):
        ...
        # 设置线性速度（单位：m/s）
        twist_msg.linear.x = 0.1  # x轴速度（前进/后退，正数向前）
        twist_msg.linear.y = 0.1  # y轴速度（左右平移，仅全向底盘支持）
        twist_msg.linear.z = 0.0  # z轴速度（上下，一般底盘不支持）
        
        # 设置角速度（单位：rad/s）
        twist_msg.angular.x = 0.0  # x轴角速度（横滚，底盘不支持）
        twist_msg.angular.y = 0.0  # y轴角速度（俯仰，底盘不支持）
        twist_msg.angular.z = 0.1 # z轴角速度（转向，正数逆时针）
